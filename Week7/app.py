...
app.py
------
DocSense AI — a RAG-based Document Question Answering chatbot.
...
from vectorstore import DocSenseVectorStore, extract_text_from_pdf, chunk_text
from chatbot import DocSenseChatbot
...
<style>
    ...
    section[data-testid="stFileUploaderDropzone"] {
        background-color: #1a1f29 !important;
        border: 1px dashed #3a3f4a !important;
    }
    section[data-testid="stFileUploaderDropzone"] * {
        color: #e6e6e6 !important;
    }
    ...
    .stat-card * { color: #f5f5f5 !important; }
...
# ---------------------------------------------------------------------------
with st.sidebar:
    st.markdown("## 🧠 DocSense AI")
    st.caption("RAG-powered document Q&A — Cohere + Pinecone")
        ...
        if st.button("⚙️ Process Documents", use_container_width=True):
            with st.spinner("Extracting, chunking, and embedding your documents..."):
                if st.session_state.store is None:
                    st.session_state.store = DocSenseVectorStore(cohere_key, pinecone_key)
                    st.session_state.chatbot = DocSenseChatbot(cohere_key)
    ...
    if st.session_state.messages:
        chat_txt = "\n\n".join(
            f"{'You' if m['role']=='user' else 'DocSense AI'}: {m['content']}"
            for m in st.session_state.messages
        )
        st.download_button("⬇️ Download chat history", chat_txt, file_name="docsense_chat_history.txt",
